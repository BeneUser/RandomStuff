...
loop:
	...
	sll $t0 $s1 2 		# $t0 = i * 4
	add $t0 $t0 $s0		# $t0 = baseAdress + i*4
	lw $a0 0($t0)
	
	# Load right_image{i} and put the value in the corresponding register
	lw $a1 36($t0)		#+36 to work from RightImage-baseAdress
	
	# Call abs_diff
	jal abs_diff
	
	#Store the returned value in sad_array[i]
	sw $v0 72($t0)		#+72 to work from sadArray-baseAdress
